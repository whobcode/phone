...
## Cloudflare Gateway (added)

This folder now includes a Cloudflare Worker gateway that exposes HTTP endpoints and forwards lookups to a running Go service. It provides:

- Rate limiting via Durable Objects
- Response caching via KV
- Task queue for asynchronous jobs

Quick start

- Configure `wrangler.jsonc`:
  - Set `UPSTREAM_BASE_URL` to your Go API base URL
  - Create a KV namespace and replace `<KV_NAMESPACE_ID>`
  - Create a queue `phone-tasks` in your account
  - Bind Durable Objects as defined (no migration code required)
- D1 database:
  - Create a D1 database `phone_db` and set its `database_id` in `wrangler.jsonc`.
  - Apply migrations: `npx wrangler d1 migrations apply phone_db`
- Run locally: `npx wrangler dev`
- Deploy: `npx wrangler deploy`

Endpoints

- `POST /api/lookup` — body: `{ "number": "+15551234567" }` → forwards to `UPSTREAM_BASE_URL/lookup?number=...`, caches result in KV
- `POST /api/task` — enqueues a background task payload to the `phone-tasks` queue and tracks status in DO
- `GET /api/health` — liveness probe

Notes

- The original Go application remains unchanged. The Worker only adds a cloud-native interface for web clients.
- Authentication: set `API_KEY` in `wrangler.jsonc` (or via Dashboard) and call with `Authorization: Bearer <API_KEY>` or `X-Api-Key`.
